...
- SwiftPM: `argmaxinc/WhisperKit`（既存）、`soffes/HotKey`（既存）、`orchetect/SettingsAccess`（既存）、`ggml-org/whisper.cpp`（新規追加）
...
| Engine 2 | whisper.cpp（`ggml-org/whisper.cpp` SwiftPM） | Kotoba-Whisper Bilingual 推論 | ggml モデルを `~/Library/Application Support/Kuchibi/models/` から読み込み |
...
| `project.yml` | `packages:` に `ggml-org/whisper.cpp` を追加 |
