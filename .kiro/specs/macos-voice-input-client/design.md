...
### Goals
- Moonshine v2を使用した低レイテンシ（100ms以下目標）の日本語音声認識
...
| ASR Engine | Moonshine v2 (SPM) | 音声→テキスト変換 | OnnxRuntime + CoreML EP |
...
- ストリーミング中はpartialTextをオーバーレイに逐次表示
...
stateDiagram-v2
    ...
    Recording --> TimedOut: 無音タイムアウト
    Processing --> OutputReady: 認識完了
    Processing --> Error: 認識エラー
    OutputReady --> Idle: 出力完了
    TimedOut --> Idle: 通知表示
    Error --> Idle: エラー通知
```
...
| 2.4 | 録音中インジケーター | Recording Overlay | OverlayWindow | 音声入力セッションフロー |
| 3.1 | Moonshineローカル音声認識 | SpeechRecognitionService, MoonshineAdapter | SpeechRecognitionService | 音声入力セッションフロー |
| 3.2 | 日本語テキスト変換 | MoonshineAdapter | SpeechRecognitionService | 音声入力セッションフロー |
| 3.3 | ローカル完結処理 | MoonshineAdapter | - | - |
...
| RecordingOverlay | Presentation | 録音中インジケーター・部分テキスト表示 | 2.4, 4.3 | SessionManager (P0) | State |
...
| SpeechRecognitionService | Domain | Moonshineを使用した音声→テキスト変換 | 3.1, 3.2, 3.3, 5.1 | MoonshineAdapter (P0) | Service |
| ClipboardService | Domain | クリップボード操作・キーストロークシミュレーション | 4.1, 4.2, 4.4 | NSPasteboard (P0), CGEvent (P1) | Service |
| MoonshineAdapter | Infrastructure | Moonshine SPMライブラリのラッパー | 3.1, 3.2 | Moonshine SPM (P0) | Service |
...
- Postconditions: `stopSession()`実行後、最終テキストが出力されstateが`.idle`に遷移
...
| Field | Detail |
|-------|--------|
| Intent | 音声バッファをMoonshineモデルに逐次投入し、テキストを生成する |
| Requirements | 3.1, 3.2, 3.3, 5.1 |

Responsibilities & Constraints:
- MoonshineAdapterを通じたモデルの初期化・管理
- ストリーミング音声データの逐次処理
- partialText/finalTextのコールバック提供
- すべての処理をローカルで完結

Dependencies:
- Outbound: MoonshineAdapter — モデル実行 (P0)
...
| Field | Detail |
|-------|--------|
| Intent | Moonshine SPMライブラリをSpeechRecognitionServiceのインターフェースに適合させるアダプター |
| Requirements | 3.1, 3.2 |

Dependencies:
- External: Moonshine SPM — ASRエンジン (P0)
...
Implementation Notes:
- 日本語モデル使用時は`max_tokens_per_second`を13.0に設定
- モデル名のデフォルト: `moonshine-tiny-ja`（Tinyモデルで軽量動作を優先）
- OnnxRuntime + CoreML Execution Providerを使用しApple Silicon上で最適化
